{{-- resources/views/user/homepage.blade.php (or similar) --}}
...
<main class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <section class="mb-16">
        ...
        <div id="featuredProducts" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {{-- Use @forelse to handle empty $products --}}
            ...
            @empty
                {{-- Placeholder for when there are no featured products --}}
                <div class="col-span-full text-center py-12"> {{-- Span all columns --}}
    ...
    <section class="mb-16">
        ...
        <div class="flex max-w-full gap-4 overflow-x-auto pb-4 custom-scrollbar">
            {{-- Use @forelse for categories too (good practice) --}}
            @forelse ($categories as $category)
                <div class="min-w-[120px] flex-shrink-0"> {{-- Added flex-shrink-0 --}}
                    <a href="/products?category={{ $category->category_name }}" class="block bg-soft-sand-beige hover:bg-muted-sage-green dark:bg-warm-black dark:hover:bg-antique-gold rounded-lg shadow-md text-center p-4 transition-colors duration-200 h-full flex items-center justify-center"> {{-- Added h-full flex items-center justify-center --}}
                        <p class="text-warm-black dark:text-warm-white font-medium">{{ ucfirst($category->category_name) }}</p>
                    </a>
                </div>
            @empty
                 {{-- Placeholder for when there are no categories --}}
    ...
    <section class="mb-16"> {{-- Added margin-bottom --}}
        ...
        <div id="newArrivals" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
             {{-- Use @forelse to handle empty $products (again, using the same variable) --}}
            ...
            @empty
                {{-- Placeholder for when there are no new arrivals (or $products is empty) --}}
                <div class="col-span-full text-center py-12"> {{-- Span all columns --}}
                     <h3 class="mt-2 text-xl font-medium text-muted-sage-green dark:text-muted-sage-green-darker">
